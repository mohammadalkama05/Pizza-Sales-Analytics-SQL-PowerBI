...
SELECT 
    pizza_types.category, COUNT(order_details.quantity) AS quantity
FROM
    pizza_types
        JOIN
    pizzas ON pizza_types.pizza_type_id = pizzas.pizza_type_id
...
SELECT 
    round(AVG(order_quantity),0) as avg_pizzas_per_day
FROM
    (SELECT 
        orders.order_date, SUM(order_details.quantity) as order_quantity
    FROM
        orders
...
from pizza_types join pizzas on pizza_types.pizza_type_id = pizzas.pizza_type_id join order_details on
